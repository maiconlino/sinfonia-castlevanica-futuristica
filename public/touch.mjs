...
export class TouchControls {
  ...
  beginAction(e,el){if(!this.accept(e))return;const action=el.dataset.action;if(!['jump','attack','magic','dash','potion','interact','swap','transform'].includes(action))return;this.capture(el,e,action);if(action!=='attack')this.pulses.add(action)}
  held(action){return [...this.pointers.values()].some(p=>p.action===action)}
  sample(){const value={left:this.direction<0,right:this.direction>0,attack:this.held('attack'),jump:this.pulses.has('jump'),magic:this.pulses.has('magic'),dash:this.pulses.has('dash'),potion:this.pulses.has('potion'),interact:this.pulses.has('interact'),swap:this.pulses.has('swap'),transform:this.pulses.has('transform'),jumpHeld:this.held('jump')};this.pulses.clear();return value}
